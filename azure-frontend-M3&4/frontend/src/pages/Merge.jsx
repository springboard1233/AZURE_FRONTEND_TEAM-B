...
import { getRawUsage, getRawExternal, getCleanedRows, downloadCleanedCsv } from '../lib/api'
...
export default function Merge() {
  ...
  useEffect(() => {
    async function run() {
      // Load cleaned rows directly; also reconstruct merge from raw for comparison
      const [u, e, cleaned] = await Promise.all([
        getRawUsage(),
        getRawExternal(),
        getCleanedRows(),
      ])
      ...
      cleaned.forEach(r => { if (r.date) r.date = String(r.date).slice(0, 10) })
      setUsage(u)
      setExt(e)
      setMerged(cleaned)

      // Build simple left-join by date for pre-merge comparison
      const byDate = new Map()
      e.forEach(r => byDate.set(String(r.date), r))
      const mergedRows = u.map(r => ({ ...r, ...(byDate.get(String(r.date)) || {}) }))
      // Keep a secondary copy if desired later (not used now)
    }
    run()
  }, [])
