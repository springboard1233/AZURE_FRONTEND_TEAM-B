...
import { getCleanedRows, getRawUsage } from '../lib/api'
...
export default function EDA() {
  ...
  useEffect(() => {
    async function run() {
      // Prefer cleaned merged rows for full fidelity; fallback to raw usage
      try {
        const rows = await getCleanedRows()
        rows.forEach(r => { if (r.date) r.date = String(r.date).slice(0, 10) })
        setUsage(rows)
        setFiltered(rows)
      } catch (e) {
        const rows = await getRawUsage()
        rows.forEach(r => { if (r.date) r.date = String(r.date).slice(0, 10) })
        setUsage(rows)
        setFiltered(rows)
      }
